...
K-023 | Convention | 文档 | 文档体系已重组（00-导航 / 01-规范 / 02~08 分域 + archive）；改文档前先读规范 | docs/01-文档规范.md | active | 2026-09-21 | 用户要求
K-024 | Convention | 推送 | 推公开仓库前必扫 `.env`/`*.db`/`evidence/` 敏感数据；本机 git 死代理须 `-c http.proxy=` + 清 `HTTP(S)_PROXY` | memory/logs/2026-09-21.md#20:10 | active | 2026-09-21 | 实测
K-025 | Pitfall | 前端·构建期env | `.env.production` 曾被 `.gitignore` 忽略 → `git clone` 部署时 `VITE_ROUTER_HISTORY`=undefined → `getHistoryMode` 的 `undefined.split` 启动即崩白屏（已修） | memory/logs/2026-09-23.md | active | 2026-09-23 | 白屏事故
K-026 | Convention | 部署·保数据 | 数据在 `server/data/`：同目录 `git pull`+重建才保留；新目录 clone 会灌种子演示数据；升级前先 `backup-db.sh` | docs/06-部署/校区部署与升级.md#5 | active | 2026-09-23 | 用户提问
