...
- 其余保持 `requireRole("admin")` 不动、本次**不开放**给 teacher 的路由：`routes/period-times.js` 的写端点、`routes/teaching-assignments.js` 全部、`routes/sessions.js` 的生成 / 停课 / 挪课 / 代课 / 回填 / 报告端点（详见 §3.2 权限列）。
...
- 仅 **`status='待上课'`** 可挪课（Q4）：生成新课次 `origin='挪课'`、`status` 按日期规则，写 `old.related_session_id = new.id`、`new.related_session_id = old.id`（**同一事务**内）。
- **不成环的保证**：`已挪课` 是**终止态**（不再可被调），所以每个课次在关联链上**至多出边为 1**；且挪课**只从 `待上课` 出发**、目标是**新建的孤立节点**（其 `related` 必为 NULL）。故关联图恒为若干**独立二元对**，**结构上不可能成环**。
- 防御：挪课前断言 `new` 尚不存在（新建）、`old.status==='待上课'`、`old.related_session_id IS NULL`。
...
| `server/src/routes/sessions.js` | 课次路由：列表 / 周视图 / 详情 / 预览 / 生成 / 回填 / 报告 / 停课 / 恢复 / 挪课 / 代课 / 手工新增 |
...
| `src/views/attendance/sessions/components/SessionDetailDrawer.vue` | 课次详情抽屉（点名 / 课评 / 课消 / 挪课记录 四页签） |
...
CREATE TABLE class_sessions (
  ...
  related_session_id    INTEGER REFERENCES class_sessions(id) ON DELETE SET NULL, -- 挪课双向关联
...
| POST | `/sessions/:id/reschedule` | admin | `{session_date,period,room_id?}` | 挪课（Q4 仅 `待上课`；新建 + 双向关联） |
...
sequenceDiagram
    ...
    alt 原课次非"待上课"
      ...
      R->>DB: BEGIN
      R->>DB: INSERT 新课次(origin=挪课,status 按日期)
      R->>DB: old.related_session_id=new.id, old.status=已挪课
...
- **验收点**：
  ...
  3. 状态视觉映射正确（待上课/已上课/已停课删除线/已挪课虚线 + 角标）；样式**全部走 token**，无硬编码色值。
  4. 生成弹窗三步（选学期→预览→确认）；确认后跳"确认后所在周"。
  5. 详情抽屉四页签（点名/课评/课消/挪课记录）；点名**自动带出当前课次**，不新增"去填课次"入口。
...
| 课次状态 | `待上课 / 已上课 / 已停课 / 已挪课 / 已取消`（前端 tag：info / success / danger·灰 / warning / info） |
| 课次来源 | `模板生成 / 挪课 / 补课 / 手工` |
