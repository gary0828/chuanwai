...
const config = {
   ...
   * 工作台自动回退规则引擎，不阻塞主流程。
   *
   * 配置来源（2026-09-23 起）：以「AI 配置中心」页面（`/#/ai-admin`）写入的
   * 数据库 `settings` 表为准（见 `utils/aiSettings.js`，DB 值覆盖此处默认值）。
   * 下方 `process.env.LLM_*` 仅作**末位兜底默认值** —— 不再由 .env / compose 注入。
   *
   * 安全约定：Key 只存服务端（数据库 / 进程环境），**绝不下发前端**
