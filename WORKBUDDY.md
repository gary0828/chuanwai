...
## 四·五、数据资产化规则（2026-09-21 新增，不可违反）

> 依据 [[ADR-008-数据资产化与AI产出层架构]]。这是**产品方向级**的架构约定，不是实现细节。

1. ★★ **系统是唯一数据源，也是唯一仓库**。所有业务数据与 AI 产出**必须**落在本系统内；**禁止**把系统数据以「Dify 里也有」为由不做本地沉淀。
2. ★★ **文件不入库**：数据库**只存元数据索引**（`student_id` / `type` / `file_path` / `size` / `created_at`），**文件本体存磁盘** `server/data/assets/{students,classes,teachers}/`。**禁止**把 PPT / PDF / 图片的二进制内容写入 SQLite（BLOB）。
3. ★★ **AI 产出必须落回自有库**：Dify 是**加工厂（只读检索）**，**不是仓库**。其知识库无「按学员 ID 查历史报告」能力、`dify/volumes/` 重建可能丢失、向量检索做不到精确查询 —— 把结果留在 Dify = 取不出 + 会丢 + 查不准。
4. **知识库三层分类**：业务数据 → 系统 SQLite；参考资料（教学法 / 话术 / SOP / 学科知识）→ Dify 知识库（只读）；AI 产出 → 系统（索引入库 + 文件落盘）。
5. **判据**：凡「**某个具体学员 / 班级**」的数据 → **系统**；凡「**通用知识 / 资料**」→ **Dify 知识库**。
6. **取数仍走网关**：Dify 取数据只能走 `/api/agent/*`，**脱敏后**送模型（沿用 ADR-004 / ADR-007）。密钥仍**服务端代持**，前端零密钥。
7. **产出层分期**：xlsx（已有）→ PDF → PPT → 视频（**暂缓**）。**禁止**一次性重构架构；一个小场景验证成功 → 复用 → 再推广。
8. **采集铁律**：沉淀一切数据 **≠** 加重老师负担。**新增表单 = 老师不用**；采集入口必须嵌在**已有页面**里；**永不强制**。
9. **备份范围**：`server/data/` 整个目录（含 `attendance.db` **与 `assets/`**）必须一起备份。**只备份 db 会丢文件**。
10. **磁盘规划**：文本类 32 GB 够用；**若要启用视频产出需扩至 100 GB+**（视频已暂缓）。
...
- **P0 模块优先于一切。**
- **每个功能都必须经过浏览器验证。**
- ★ **数据资产化铁律（2026-09-21）**：系统是唯一数据源与唯一仓库；**文件不入库**（索引进库、文件落盘）；**AI 产出必须落回自有库**，不留在 Dify。详见 §四·五与 [[ADR-008-数据资产化与AI产出层架构]]。
