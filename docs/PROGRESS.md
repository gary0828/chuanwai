...
- [x] **前端全站视觉改版（2026-09-18）** —— 「教务台」方向，**功能零变更**
  ...
  - **已知遗留**（详见 `docs/前端视觉规范-2026-09-18.md` §八）：`el-empty` 未逐页换成 `AppEmpty`（插画已全局隐藏、字号字色已统一，视觉一致但组件未统一）；错误页 403/404/500 保持原结构；`/ai-admin` 不进标签页（pure-admin 标签逻辑基于菜单匹配，**属框架既有行为、非本次改版引入**）；深色模式 token 已同步但**未做浏览器验证**
