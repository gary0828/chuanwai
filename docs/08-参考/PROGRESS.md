...
> 历史进度流水（按时间倒序）。**新条目请写 `CHANGELOG.md`**，本文件只作存档。
> 早期条目沿用旧术语（如「学生」），与现行 [术语表](../01-文档规范.md) 不一致属正常。

# 项目进度（PROGRESS）

> 每次会话开始读取本文件，结束更新。最近更新：**2026-09-21**
> 数据库版本：**v19**（2026-09-21 校准：原写 v18 已过期，L2 待办已迁移到 v19） ｜ e2e **80/80** ｜ analytics **22/22** ｜ docker-verify **9/9** ｜ P0 浏览器 **31/31** ｜ 前端全站扫描 **29/29（0 控制台错误）**
...
- [x] **记忆系统 v2（2026-09-21）** —— 开发期工具，**不进任何部署产物**
  - 起因：项目内容膨胀，跨会话要重复解释背景
  - 结构：新建 `memory/`（git 追踪）——`INDEX.md` 唯一入口 / `CURSOR.md` 当前进度 / `PROTOCOL.md` 读写协议 / `logs/` 日志 / `knowledge/_index.md` 知识索引（22 条，指向 `../conventions` 与 ADR）/ `tasks/` 三态 / `archive/` / `check.mjs` 体检脚本
  - 跨模型生效：新增 `AGENTS.md` / `CLAUDE.md` / `.cursorrules` 三个入口指针，各 ≤21 行；`WORKBUDDY.md` §10.3 启动流程改为指向 `INDEX.md`（**不再要求通读本文件**）
  - 部署排除：`.dockerignore` 新增 `memory` + 三个入口文件；`check.mjs` §5 部署护栏会断言 9 个部署配置文件不含 `memory` 引用（已做负向测试验证）
  - 与 09-21 另一套「conventions 七领域分层」（`4b602ae`）**融合而非替换**：领域分层管「知识归哪」，本系统管「入口 / 进度 / 流转 / 瘦身」，两者正交；conventions 与 ADR **一个文件都没迁**
  - 实测：启动只读 6 KB；`node memory/check.mjs` 首次运行即抓到 `CURSOR.md` 超限并已裁剪
...
- [x] **待办功能 · L3 自动生成（2026-09-21）** —— 4 类业务事件自动转待办，**零新迁移**
  ...
  - **文档**：`server/database.md`（todos 章节补「自动生成（L3）」）· `../04-API/API.md` · `../04-API/openapi.yaml`（新增 `/api/todos/generate`）· `ROADMAP.md`
...
- [x] **待办功能 · L2 手工待办（2026-09-21）** —— 迁移 v19 + 双端页面，**双端共用一张表**
  ...
  - **迁移 v19**（`019-todos.js`）：新增 `todos` 表（14 字段 / 3 索引）。★ **`(source_type, source_ref_id, owner_id)` 部分唯一索引 = L3 自动生成的幂等去重键**，提前建好 → **L3 无需再动表结构**。**既有业务表一张未动**
  - **★ 为什么一张表而不是两套**：待办本质同构；拆两套则生成逻辑/权限/完成语义都要写两遍，且两端数据无法互通
  - **后端** `routes/todos.js`（GET/POST/PUT/DELETE）+ 挂载 + 审计模块名「待办」
  - **★ 权限边界在路由内的归属校验，不在路径**：`canManage` = owner 或 admin；
    **teacher 无论传什么 `scope` 一律收敛为「只看自己」**；`admin` 可 `scope=all` 看全部、可指派他人、可改他人待办。
    `middleware/auth.js` 给 `ai_agent` **读写同权**放行 `/api/todos`（老师在工作台既要看也要改）
  - **三处 UI**：教务端左侧菜单「待办」（rank 0，排最前）+ 铃铛「待办」tab（与「通知」并列）+ **AI 工作台**「我的」→「我的待办」
  - **★ 工作台页踩坑（两个错叠加，整页像"白板"）**：① 用了**原生 `<input>/<select>`**，而全工作台其它页一律 `el-input/el-select`；② scoped CSS 里写了 `var(--brand)`/`var(--line)`/`var(--danger)` —— **这些变量在工作台不存在**（实际是 `--c-primary`/`--c-border`/`--c-danger`），导致自定义样式**全部静默失效**。已照抄 `Settings.vue` 重写（`el-input`/`el-select`/`el-date-picker`/`el-radio-group` + `.form-row` + `.rec-head/.rec-row` 网格 + 正确的 `--c-*` 变量）
  - **验证**：
    - 迁移独立校验 `check-migration-019.mjs`：字段/索引/**幂等键生效**/CHECK 全通过
    - 容器日志 `[migrate] 数据库已升级到 v19（待办）`；`user_version=19`
    - `verify-todos.mjs` API 权限 **15/15**
    - `ui-todos.py` UI（admin）**14/14**
    - ★ `ui-todos-teacher.py` **权限差异实证（教师 vs 管理员对照）15/15** —— 教师：看不到管理员的待办 · 筛选区无「负责人」下拉 · 新建对话框无指派项 · 铃铛只含自己的；管理员：能看全部 · 有「负责人」筛选与指派
    - ESLint 0 错 · 三镜像构建 EXIT=0 · 契约门禁 87 个 `$ref` 全解析
  - **文档**：`server/database.md`（v19 + todos 表结构与权限模型）· `../04-API/API.md` · `../04-API/openapi.yaml`（新增 `Todos` tag + 3 个路径）
  - **后续**：**L3 待办·自动**（幂等生成器 + 4 类事件：余额/课时预警、连续缺勤、线索待跟进、课评/知识点欠录）

- [x] **顶栏铃铛接真实公告（L1，2026-09-21）** —— 删掉写死的演示数据，改接 `/api/notices`
  - **背景**：铃铛此前读 `src/layout/components/lay-notice/data.ts` 的**模板演示数据**（"小铭 评论了你"、"指派小铭于 2024-06-18…"、"开发多租户管理"），从未请求后端 —— 上线后员工会看到这些假消息。★ **09-14 就发现了，却被记入本文件（"已完成"文档）→ 失踪 7 天**；本次起待决策事项一律只进 `ROADMAP.md`
  - **★ 需求经 `grill-me` 追问 11 轮定稿**，是首个完整走 `feature-dev-flow` 流程的功能（需求澄清 → 扫描+计划 → 开发+环境 → 文档推送）
  - **★ 关键核查：砍掉「消息」tab**。原计划把「通知记录 `notifications`」当消息用，核查后发现它页面描述是 **「已发送的家校通知存档」**（发件记录，非收件箱）；写入语句只有 `student_id/type/title/content/date/parent_name`，**没有收件人概念**；`target_user_id` 字段在迁移 010 定义但**全项目路由从未使用**（死字段）。放进铃铛会误导用户，且与「待办」定位重叠 → 砍掉。**铃铛最终形态 = 2 tab（通知 + 待办）**
  - **改动 4 个文件**（★ **零后端改动、零迁移、零 nginx/compose 改动** —— 已逐项核查确认，非遗漏）：
    - `lay-notice/types.ts` **新建**（只放类型，不含数据）
    - `lay-notice/index.vue` **重写**：改调 `getNoticeList`；红点 = 最近 7 天「已发布」公告数
    - `lay-notice/components/{NoticeList,NoticeItem}.vue`：import 由 `../data` 改 `../types`
    - `lay-notice/data.ts` **删除** → 3 个第三方外链 `xiaoxian521.github.io` **一并消失**（**违反 ADR-002「不出网」的隐患清零**）
  - **★ 实现要点**（都是踩过的坑）：
    - `/api/notices` 列表接口**不按 status 过滤**（管理页需要看到「下架」的）→ 铃铛必须**自行过滤 `status === '发布'`**
    - `created_at` 形如 `YYYY-MM-DD HH:mm:ss`（localtime），`new Date()` 前把 `-` 换 `/` 以提兼容
    - 拉取失败**静默降级**（红点归零 + 空态），不弹错误打扰用户
    - 红点走**时间基准**而非已读基准 —— 公告表无 per-user 已读记录，走已读需新建 `notice_reads` 表 + 迁移，L1 刻意不做（见 `ROADMAP.md` 决策）
  - **验证**：ESLint **0 错** ｜ `vue-tsc` 报的 ~20 个错误**无一在 `lay-notice/`**（全是既有问题，本次**零新增**）｜ 生产构建 **28.97s / 3.57 MB** ｜ 新增 `_verify_test/ui-bell-notice.py` **9/9**（含「下架公告不显示」「无演示数据残留」「无第三方外链」）｜ **用户浏览器实测通过**
  - **代码审查（五轴）留 2 项可选未改**（为不让已通过的用户验证失效，本次不动）：① `onMounted(load)` 与下拉展开可能并发触发 `load()`，无在途守卫（两次响应同构，影响极低）；② 行映射用 `any`，未给 API 响应建显式类型边界（与项目既有约定一致）
  - **后续**：L2 待办·手工、L3 待办·自动 —— 见 `ROADMAP.md`「五之二」
...
- [x] **`openapi.yaml` 补齐成长路径契约（2026-09-20）** —— 补 8 个路径 + 8 个 schema
  - **背景**：上一轮只更新了 `../04-API/API.md`，`openapi.yaml` 里 `/api/growth/*` **一个都没有**（42 个路径中缺失），属文档门禁欠账
...
- [x] **前端全站视觉改版（2026-09-18）** —— 「教务台」方向，**功能零变更**
  - **方向定调**（四项设计上下文经确认）：中度重塑 / 专业信赖 / 台式+平板并重 / 先样板页再全量。主色由 Element 出厂 `#409EFF` 换为深靛蓝 `#1F5C99`。设计上下文持久化在根目录 `.impeccable.md`，人类可读规范见 `../03-开发指南/前端视觉规范.md`
  ...
  - **已知遗留**（详见 `../03-开发指南/前端视觉规范.md` §九）：`el-empty` 未逐页换成 `AppEmpty`（插画已全局隐藏、字号字色已统一，视觉一致但组件未统一）；错误页 403/404/500 保持原结构；`/ai-admin` 不进标签页（pure-admin 标签逻辑基于菜单匹配，**属框架既有行为、非本次改版引入**）；深色模式 token 已同步但**未做浏览器验证**
...
- [x] **服务端大模型接入 + 端到端闭环（2026-09-14 晚）**
  ...
  - 文档同步：`../04-API/API.md` §5 更新 `AI_WORKBENCH_URL`（5300→8082）、CORS 同源反代说明、`LLM_MODEL` / `LLM_MAX_TOKENS` 口径；`server/.env.example` 补 AI 配置段；`README.md` 新增「AI 教学工作台与大模型配置」
...
- [x] **AI 教学工作台原型（方案 C：逻辑独立、部署统一）**（2026-09-14）
  ...
  - **验证**：后端三文件语法检查通过 ｜ 免登链路专项 **18/18**（`_verify_test/verify-ai-sso.mjs`：未登录取票 401、票据载荷不含身份字段、重放 401、篡改签名 401、缺票 400、非 JWT 401）｜ 现有前端生产构建通过（24.5s / 3.53 MB）｜ `../04-API/openapi.yaml` 解析通过（35 个路径，已含 `/api/ai/*`）
...
- [x] **全仓过度设计审计与清理（ponytail-audit）**（2026-09-14）
  - 审计报告：`../archive/过度设计审计报告-2026-09-14.md`（含 10 项发现、5 处误报排除记录、执行结果）
...
- [x] **文档清理与归位**（2026-09-14）
  - 删除 `../README.en-US.md`（vue-pure-admin 模板自带的英文 README，与项目无关，根 README 亦未引用）
  - 同步修正 `../00-导航.md`（数据库版本 v14→v15、补 ADR-006 索引、更新回归基线、新增「评审报告归档」表）与 `../03-开发指南/项目地图.md`（源码规模、目录地图去掉 `mock/`、补 `utils/grade.js`、常用命令改为可复现脚本）
...
- [x] **创建 `../04-API/API.md`**：API 唯一权威文档。含通用约定、错误码、认证、角色数据权限、**全量接口总览（110+ 端点）**、P0 模块接口详解、analytics 信封与指标口径、新增 API 流程（2026-09-11）
- [x] **创建 `../04-API/openapi.yaml`**：OpenAPI 3.0 机器可读规范，覆盖认证 + 4 个 P0 模块 + 通知 + analytics，含复用 schema / responses（2026-09-11）
...
## 全面测试与缺陷修复（2026-09-12 下午，完整报告见 `../archive/全面测试报告-2026-09-12.md`）
...
4. **README 与真实实现存在偏差**（已修正其中一处）：e2e 断言数 61 → 80。另需注意：提示词第三节给出的 `POST /api/attendance/batch` 请求体含 `class_id`、响应含 `hours_deducted/notifications_created`，而**真实实现不含 `class_id`**（班级由学生档案决定），响应为 `{ count }`。`../04-API/API.md` 以**真实实现**为准。
...
> 完整报告见 **`../archive/上线评估报告-2026-09-12.md`**（含全部 4 阻断 / 12 高 / 18 中 / 8 低问题清单与上线门禁）。
...
15. **线索转化建单缺 `total_hours`，转化学员永不扣课时、永不确认收入**（`server/src/routes/leads.js:296-313`）
    ...
    - 且 `../04-API/API.md` 原文档声称 convert 支持 `total_hours` 参数，**后端静默忽略** → 前端按文档接入必然踩坑。
    - 处置：需业务方二选一并落地——① 转化即报班（补写 `total_hours`）；② 转化仅建档（修正文档 + 前端下线该字段）。当前已把 `../04-API/API.md` 改为**如实描述实现**并标注该缺口。
...
40. **`../04-API/openapi.yaml` 覆盖不足一半**：后端共 115 个路由处理器，openapi 未全量登记。
...
| **B2** | 线索转化明确为「仅建档」 | `../04-API/API.md` §三 改写为如实语义 + 补录指引；`src/views/recruit/leads/index.vue` 转化弹窗加 `el-alert` 提示、修正恒为空的标题、成功提示补充后续步骤。**前端本就只提交 4 个字段**，与后端一致 |
...
- `server/src/routes/schedules.js` 的 `POST /api/schedules/check-conflict` 未在 `../04-API/API.md` 登记 → **文档待补**
...
| `../04-API/API.md` | §5.7 安全设计表补「已移除浏览器直连旁路」「用量集中可审计」两行 |
...
### 本轮定案（全文见 `../07-架构与决策/ADR/ADR-008-数据资产化与AI产出层架构.md`）
...
- 新建 `../07-架构与决策/ADR/ADR-008-数据资产化与AI产出层架构.md`
- `../07-架构与决策/ROADMAP.md`：D7 标注修订 ／ 新增 D15 ／ §二 Non-goals 解除 RAG 一条 ／ 新增 Phase 6
- `../00-导航.md`：ADR 索引新增 ADR-008 ／ 阶段速查新增 Phase 6 ／ 忽略规则更正为仅 `.aiignore`
...
## 2026-09-26 · 菜单重构（方案 A）+ 出勤率口径统一 + 备份实现统一

**动机**：用户反馈「让别的 AI 做了些功能，出现很多重复，而且菜单太乱」。
对全项目做扫描（35 个页面 / 31 个后端路由 / 双端），结论与直觉相反 ——
**真正的功能重复只有 2 处**（都不在菜单上、老师感知不到），
但**菜单分组确实乱**：8 个一级分组里有 4 个是"什么都往里塞的筐"。
扫描报告见 `功能重复扫描与菜单重构方案-2026-09-24.md`（含 35 项菜单现状全表与 A/B 两方案对照）。

### 一、菜单重构（方案 A，`server/src/routes/auth.js` 的 `ROUTES`）

| 改前 | 改后 |
|---|---|
| 考勤管理（**8 项**，混进了课表体系） | 考勤管理（**4 项**：登记/记录/请假/统计） |
| 数据管理（**7 项**，筐式分组） | **拆解消失** → 学员管理（学生/班级/课程/学期）+ 排课配置并入新组 |
| —— | ★ 新增 **排课与课表**（周课表/排课模板/调课审批/补课管理/节次时间/任课关系） |
| 招生管理（1 项）+ 财务下的「报班管理」 | 合并为 **招生与报名**（线索 → 报班，同一业务链） |
| 教学结果 / 家校管理 / 财务管理 | 更名 **教学成果 / 家校沟通 / 财务** |
| 员工账号（顶层独立项） | 移入 **系统管理**（与其它 admin 工具同类） |

- admin：9 组（含「待办」独立项）；teacher：6 组，**不下发**财务 / 招生与报名 / 系统管理
- ★ **所有子项 path 一律未改** —— 路由地址不变，老师收藏的页面不会失效
- 已核实前端不依赖父子 path 前缀（`getParentPaths` 存父级 path、`addAsyncRoutes` 不拼接路径、子路由按绝对路径独立注册）

### 二、出勤率口径统一（真重复 #1）

- **问题**：同一指标「出勤率」有两套公式并给出不同答案 ——
  `attendance.js` = `(总数−缺勤)/总数`（请假计入分母）vs `analytics.js` = `实到/(总数−请假)`（剔除请假）。
  例：total=100 / absent=5 / leave=10 → 一个 95%，另一个 94.4%
- **处置**：统一为 `(总数−缺勤)/总数`（与统计报表、AI 工作台 `engine.ts` 一致，2:1 多数派），
  新建 `server/src/utils/attendance-rate.js` 收敛 **9 处**内联公式
  （`attendance.js`×6 / `reports.js`×1 / `analytics.js`×1 / `agent.js`×1），并删除 `analytics.js` 的 `attDenom`
- **横向扫描的额外收获**：原以为 7 处，实扫出 9 处（`agent.js` 工作台出口、`attendance.js` 缺勤预警被漏数）；
  另同步修正了 `analytics.js` 的**指标口径字典**与 `API.md` 中记录**旧公式**的两处（口径字典若与实现不符，AI 会按错误口径解释数据）
- **口径边界**（易记错）：因 `total = 正常+迟到+早退+缺勤+请假`，故 `总数−缺勤` **含请假** →
  **出勤率 + 缺勤率 = 100%**；请假率是与之**重叠**的独立维度，**不可**与两者相加
- **数值安全**：与改造前公式做了 **20301 组样本对拍**，完全一致（无回归）

### 三、备份实现统一（真重复 #2）

- **问题**：数据库备份有两套独立实现、技术路线不同 —— 应用内 `copyFileSync`＋`wal_checkpoint`
  （`server/src/utils/backup.js`）vs 运维脚本 `VACUUM INTO`（`server/scripts/backup-db.sh`）。
  ★ 而运维脚本的注释**明确批评过** `copy` 方案会漏 `-wal` 未落页、可能拷出损坏副本 ——
  应用内用的正是被批评的做法
- **处置**：应用内改用 `VACUUM INTO`，两套统一为同一份安全实现。两处入口**都保留**
  （应用内是校区管理员唯一能自助恢复的通道；脚本是停机外的灾难恢复通道）
- 附带加固：目标文件同名时先清理；失败时清掉半成品（避免"损坏的备份"被当成可用备份恢复）

### 四、课程页「授课教师」改为展示标注

- `courses.teacher` 是**自由文本**，而任课关系（`teaching_assignments`）是外键、是"老师能看到哪个班"的权限依据；
  两者并存且冲突检测双读 → 管理员在课程页改教师可能"什么都没发生"
- 处置：字段改名 **「默认授课教师」**、placeholder 与页内提示明确"仅作展示，实际任课在【任课关系】设置"、页头补充说明
- 暂**不删** `schedules.js` 的文本级兜底分支（历史数据未确认迁完）

### 五、验证（统一入口 18080，镜像已重建）

| 套件 | 结果 |
|---|---|
| 菜单重构 API 契约（新增 `_verify_test/verify-menu-refactor.mjs`） | **51 PASS / 0 FAIL** |
| 菜单重构 UI（新增 `_verify_test/ui-menu-refactor.py`，双角色） | **32 PASS / 0 FAIL** |
| e2e 生命周期 | 80 PASS / 0 FAIL |
| 课次/任课关系 | 90/91（唯一 FAIL 为既有 P1 缺口 B7，非回归） |
| openapi 契约 | 147 个 $ref 全可解析，exit 0 |
| 备份产物 | `integrity_check = ok`、`foreign_key_check` 无违规、与主库数据一致（27 学员 / 130 考勤） |

- ★ 新增断言「**新菜单覆盖重构前全部 32（admin）/14（teacher）项**」，用代码证明**没有丢功能**
- 截图：`evidence/menu-refactor-{admin,teacher,courses}.png`

### 六、顺带修复：回归脚本的「自污染 → 自己崩」

`server/scripts/verify-sessions.mjs` 的挪课测试**每跑一次就占用一个硬编码候选时段**且无清理机制，
跑满 4 次后候选耗尽 → `freeSlot=null` → `freeSlot[0]` 直接 TypeError，**后半个脚本都没跑**（2026-09-26 实测踩中）。
已改为：① 未来 60 天动态搜索空闲时段；② 跳过带 `related_session_id` 残留的"脏"课次；
③ 找不到时 **SKIP 并写明原因**（延续 K-041：红 FAIL 只应代表真缺陷）

### 七、同步修改的文档

- `docs/04-API/API.md`：权限边界四条（菜单改为双套结构说明）+ 出勤率口径两处（含被废弃的旧公式）
- `docs/04-API/openapi.yaml`：`/api/auth/async-routes` 增补菜单结构说明
- `docs/08-参考/功能重复扫描与菜单重构方案-2026-09-24.md`：状态转「已实施」，
  并**修正一条误报**（§5 撤销「课消统计权限矛盾」——teacher 菜单本就不含财务管理，原判断有误）
